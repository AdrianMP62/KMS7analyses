...
This code available at https://github.com/AdrianMP62/KMS7analyses  

RStudio: Version 1.1.456 – © 2009-2018 RStudio, Inc. R version: `r getRversion()`  
Date and time: `r Sys.time()`

Uses the following datafiles:  
ionic composition mgl.csv (2016-17 ionic composition data)   
KMS17_invert_data.csv (2017 KMS invertebrate presence/absence data)  
KMS-VBM data.csv (all KMS invertebrate presence/absence data - 1999-2017)  
...
data.rich <- read.csv("KMS-VBM data.csv", stringsAsFactors = FALSE, check.names = FALSE)
```

```{r create new empty matrix for richness values}
rich.sums <- data.rich[0, 4:43]
rich.sums[1:12, 1] <- c("protozoans","rotifers","molluscs", "annelids","mites","cladocera","ostracoda","copepoda","amphipoda","shrimps","crabs","insects")
```

```{r use loop to calculte richness values per site for each of the twelve major taxon groups combined into one file}
row=0
for (i in c("^B","^J","^K","^L","^M","^OG","^OH","^OJ","^OP","^OT","^OX","^Q"))
  {
  data.rich.temp <- data.rich[data.rich$LOWESTIDNC %like% i, ]
temp <- colSums(data.rich.temp[, 5:43])
row=row+1
rich.sums[row, 2:40] <- temp
}
```
