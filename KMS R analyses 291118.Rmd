...
Packages <- c("vegan", "ggplot2", "stringr", "data.table", "simba", "Ternary", "RColorBrewer", "iNEXT", "eulerr", "car")
...
ggplot(Spec.area, aes(fill=Distribution, y=Species, x=Area)) +
    geom_bar( stat="identity", colour="black") + scale_fill_manual(values=c("#79CDCD", "#EEDC82")) + theme(axis.text.x = element_text(size=14, angle = 90, hjust = 1)) + theme(axis.text.y = element_text(size=14)) + theme(axis.title.y=element_text(size=15, margin = margin(t = 0, r = 20, b = 0, l = 0))) + theme(axis.title.x = element_blank()) + theme(legend.text = element_text(size=14)) +theme(legend.title = element_text(size=18)) + ylim(0, 320)
```
