...
temp <- colSums(data.rich.temp[, 5:48])
row=row+1
rich.sums[row, 2:45] <- temp
}
```
...
rich.sums[NROW(rich.sums)+1, 2:NCOL(rich.sums)] <- colSums(rich.sums[grepl("cladocera|ostracoda|copepoda", rich.sums$LOWESTIDNC), 2:45])
...
ggplot(rich.sums.list[rich.sums.list$sample %like% "1996|1999|2000|2001", ], aes(sample, richness, fill=factor(taxon,levels=c("protozoans","rotifers","other","molluscs","annelids","mites","microcrustacea","macrocrustacea","insects")))) + 
...
##Compositional richness of combined plankton and benthic samples collected in 1993 and 2003
```{r plot stacked column plot, 1993 Halse et al. 1996 data with the 2003 sample from Kachana}
ggplot(rich.sums.list[rich.sums.list$sample %like% "1993|2003", ], aes(sample, richness, fill=factor(taxon,levels=c("protozoans","rotifers","other","molluscs","annelids","mites","microcrustacea","macrocrustacea","insects")))) + 
  scale_fill_brewer(palette="Set1") +  #use RColourBrewer package to set colours
  geom_bar(stat = "identity", colour="black") + #colour=black sets borders for the bars
  scale_y_continuous(limits=c(0, 100)) + #set y-limit to 100 for consistency
  guides(fill=guide_legend(title=NULL)) + #remove legend title
...
ggplot(rich.sums.list[rich.sums.list$sample %like% "2017-B|2017-P", ], aes(sample, richness, fill=factor(taxon,levels=c("protozoans","rotifers","other","molluscs","annelids","mites","microcrustacea","macrocrustacea","insects")))) + 
  scale_fill_brewer(palette="Set1") +  #use RColourBrewer package to set colours
  geom_bar(stat = "identity", colour="black") + #colour=black sets borders for the bars
  scale_y_continuous(limits=c(0, 100)) + #set y-limit to 100 for consistency
  guides(fill=guide_legend(title=NULL)) + #remove legend title
