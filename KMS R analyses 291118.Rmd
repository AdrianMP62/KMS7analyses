...
* Ordination of Mandora plus Kimberley mound spring invertebrates  
...
invertspec <- read.csv("spec_accum2.csv", check.names = FALSE)
...
Spec.area <- data.frame("Species" = c(84,5,106,14,73,46,37,36,140,149)) #values are alternately number of species known from more than one area and then from just that area
...
ggplot(Spec.area, aes(fill=Distribution, y=Species, x=Area)) +
    geom_bar( stat="identity", colour="black") + scale_fill_manual(values=c("#79CDCD", "#EEDC82")) + theme(axis.text.x = element_text(size=10, angle = 90, hjust = 1)) + theme(axis.title.y=element_text(size=10, margin = margin(t = 0, r = 20, b = 0, l = 0))) + theme(axis.title.x = element_blank()) + theme(legend.text = element_text(size=11)) +theme(legend.title = element_text(size=14)) + ylim(0, 320) + scale_x_discrete(labels = c("Dampier","King Sound east coast","Central","Kachana","Victoria-Bonaparte"))
```


__________________________________________________________________________________________
##Ordination of combined Walyarta and KMS invertebrate data
...
samples <- gsub("-99", "Mandora 1999", samples)
samples <- gsub("-15", "Mandora 2015", samples)
samples
```

```{r nMDS ordination of KMS and Walyarta data in vegan, results=FALSE}
kmsman2.mds <- metaMDS(kmsman2, distance="jaccard", k=3)
kmsman2.pts <- as.data.frame(kmsman2.mds$points)
```

```{r}
kmsman.mds.plot <- ggplot(kmsman2.pts, aes(x=MDS1, y=MDS2)) + coord_fixed(ratio = 1)  + geom_point(aes(size=2, colour=samples)) + scale_size(guide = 'none') + guides(colour = guide_legend(override.aes = list(size=5))) #+ geom_text(aes(label = row.names(kmsman2.pts)), vjust=0.5, hjust=1)
...
```{r}
#based on 5 samples of each types collected at the same five springs
con <- c(A = 147, B = 55, C = 21, "A&B" = 26, "A&C"=11, "B&C" = 18, "A&B&C"=26) #A=KMS, B=Man-99, C=Man-15
plot(euler(con, shape = "ellipse"), edges = TRUE, quantities = TRUE, legend = list(labels = c("Kimberley", "Mandora-2015", "Mandora-1999"), cex=1.5))
```
