...
kmsman2 = as.matrix(as.data.frame(lapply(kmsman2[, 1:26], as.numeric))) #convert data to numeric
...
samples <- str_sub(samples, -2, -1)
samples <- gsub("-17", "Kimberley 2017", groups)
samples <- gsub("-99", "Mandora 1999", groups)
samples <- gsub("-15", "Mandora 2015", groups)
```

```{r nMDS ordination in vegan, results=FALSE}
kmsman2.mds <- metaMDS(kmsman2, distance="bray", k=3)
kmsman2.pts <- as.data.frame(kmsman2.mds$points)
```

```{r}
kmsman.mds.plot <- ggplot(kmsman2.pts, aes(x=MDS1, y=MDS2)) + coord_fixed(ratio = 1)  + geom_point(aes(size=2, colour=samples)) + scale_size(guide = 'none') + guides(colour = guide_legend(override.aes = list(size=5)))
