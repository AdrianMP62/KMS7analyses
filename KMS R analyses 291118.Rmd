...
ggplot(Spec.area, aes(fill=Distribution, y=Species, x=Area)) +
    geom_bar( stat="identity", colour="black") + scale_fill_manual(values=c("#79CDCD", "#EEDC82")) + theme(axis.text.x = element_text(size=10, angle = 90, hjust = 1)) + theme(axis.title.y=element_text(size=10, margin = margin(t = 0, r = 20, b = 0, l = 0))) + theme(axis.title.x = element_blank()) + theme(legend.text = element_text(size=11)) +theme(legend.title = element_text(size=14)) + ylim(0, 250) + scale_x_discrete(labels = c("Dampier","King Sound east coast","Central","Kachana","Victoria-Bonaparte"))
```
...
##Cluster analysis of Walayrta versus Kimberley mound springs
```{r}
