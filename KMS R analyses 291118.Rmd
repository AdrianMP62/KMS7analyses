...
```{r extract list of sample types}
region <- as.list("Kimberley","Kimberley","Kimberley","Kimberley","Kimberley","Kimberley","Mandora")

```

```{r nMDS ordination in vegan, results=FALSE}
kmsman2.mds <- metaMDS(kmsman2, distance="bray")
kmsman2.pts <- as.data.frame(kmsman2.mds$points)
```

```{r}
data2.mds.plot <- ggplot(kmsman2.pts, aes(x=MDS1, y=MDS2)) + coord_fixed(ratio = 1)  + geom_point(aes(size=2)) + geom_text(aes(label = samples), cex=3, vjust=2, hjust=1)
print(data2.mds.plot)
```
