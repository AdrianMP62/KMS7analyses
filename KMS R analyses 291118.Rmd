...
```{r}
invert.spec.acc <- poolaccum(invertspec)
plot(invert.spec.acc)
invert.spec.acc
```
...
```{r}
invertspec <- t(invertspec)
invert.spec.acc2 <- iNEXT(invertspec, q=0, endpoint=200, datatype="incidence_raw") #q=0 specifies species richness
